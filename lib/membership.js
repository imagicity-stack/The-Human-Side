...
import { sendWelcomeEmail, sendVoicesUnheardWelcomeEmail } from "./mailer";
...
export const PROGRAMS = {
  membership: {
    ...
    collection: "members",
    counterDoc: "members",
    settingsDoc: "registration",
    defaultFee: 999,
    idPrefix: "THS",
    idPad: 3,
    welcomeEmail: sendWelcomeEmail,
    subjectLabel: "Volunteer Membership",
  },
  "voices-unheard": {
    key: "voices-unheard",
    collection: "voicesUnheard",
    counterDoc: "voicesUnheard",
    settingsDoc: "voicesUnheard",
    defaultFee: 799,
    idPrefix: "VU-2026-",
    idPad: 3,
    welcomeEmail: sendVoicesUnheardWelcomeEmail,
    subjectLabel: "Voices Unheard 2026",
  },
};

export function getProgram(program) {
  return PROGRAMS[program] || PROGRAMS.membership;
}

export async function getCurrentFee(program = "membership") {
  const cfg = getProgram(program);
  try {
    const snap = await getDb().collection("settings").doc(cfg.settingsDoc).get();
...
export function cleanRegistration(body = {}, program = "membership") {
  ...
  if (program === "voices-unheard") {
    return {
      ...
      classGrade: str(body.classGrade, 50),
      section: str(body.section, 20),
      tshirtSize: str(body.tshirtSize, 10),
      guardianName: str(body.guardianName, 100),
      guardianRelation: str(body.guardianRelation, 50),
      guardianPhone: str(body.guardianPhone, 30),
      guardianEmail: str(body.guardianEmail, 200),
      emergencyName: str(body.emergencyName, 100),
      emergencyPhone: str(body.emergencyPhone, 30),
      medicalNotes: str(body.medicalNotes, 500),
      hearAbout: str(body.hearAbout, 100),
      motivation: str(body.motivation, 2000),
      consents: arr(body.consents),
    };
  }
...
export async function finalizeMembership(registrationId, paymentId, orderId, program = "membership") {
  const cfg = getProgram(program);
  const db = getDb();
  const docRef = db.collection(cfg.collection).doc(registrationId);
  const counterRef = db.collection("counters").doc(cfg.counterDoc);
  ...
  const out = await db.runTransaction(async (tx) => {
    ...
    const next = last + 1;
    const memberId = cfg.idPrefix + String(next).padStart(cfg.idPad, "0");
  ...
  if (!out.already) {
    try { await cfg.welcomeEmail(out.member); }
